...
// jspdf-autotable 3.x's peerDependency only covers jsPDF ^2.5.1, but this app
// runs jsPDF ^4.2.1. The standalone `autoTable(doc, opts)` function export is
// not callable against a v4 doc instance — it throws "autoTable is not a
// function" and silently aborts the whole PDF export. Registering the plugin
// once via applyPlugin() restores `doc.autoTable(opts)` as a working instance
// method, which IS compatible across jsPDF major versions.
applyPlugin(jsPDF);
import { getService, SECTIONS } from '../data/services.js';
...
function drawFooter(doc, W, H, agencyName, agencyWebsite, agencyEmail, agencyPhone, pageNum, totalPages, agencyAddress = '', agencyCityState = '', whiteLabelPlan = false) {
  ...
  const accent = hexToRgb(doc.__accentHex || '#C6E404');
  ...
  if (!whiteLabelPlan) {
    ...
    doc.text('Powered by Invoice King.io', W / 2, H - FOOTER_H + 24, { align: 'center' });
  }
  if (totalPages > 1) {
...
// Builds the jsPDF document object and returns it without saving/downloading.
// Shared core used by both exportPDF() (browser download) and the
// server-side email-attachment path (server/utils/pdf.js), so there is
// only one place that contains the actual quote PDF layout and totals
// math — duplicating this a second time risks the same class of bug
// found repeatedly elsewhere this session (independent copies of the
// same calculation silently drifting out of sync with each other).
export function buildQuotePDFDoc(state) {
  const {
  ...
  doc.setFontSize(7);
  doc.setTextColor(...INK_MUTED);
  const footerText = `Invoice King — Invoicing. Simplified. Own Your Cash Flow. | ${agencyEmail || 'support@invoiceking.app'} | This document was generated by Invoice King and represents a quote/invoice for services rendered.`;
  doc.text(footerText, W / 2, footerY, { align: 'center', maxWidth: W - 24 });

  return doc;
}

// Default filename used by both the browser download and the server-side
// email attachment, so a PDF saved by a user and a PDF emailed to a client
// are named identically for the same quote.
export function defaultQuotePdfFilename(clientName) {
  return `Quote_${(clientName || 'Client').replace(/\s+/g, '_')}_${new Date().toISOString().slice(0, 10)}.pdf`;
}

// Browser-only: builds the PDF and triggers a download via jsPDF's save(),
// which uses DOM APIs that only exist in a browser context.
export function exportPDF(state) {
  const doc = buildQuotePDFDoc(state);
  doc.save(defaultQuotePdfFilename(state?.clientName));
}
