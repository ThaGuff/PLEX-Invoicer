  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        background: isOwn ? '#C6E404' : '#C6E404',
              ...
              <span key={i} style={{ whiteSpace: 'pre-wrap' }}>
                {parts.map((p, j) => {
                  ...
                  const cleanMention = p.includes('@', 1) 
                    ? '@' + p.slice(1).split('@')[0] 
                    : p;
                  return <strong key={j} style={{ color: 'var(--accent, #C6E404)', fontWeight: 700 }}>{cleanMention}</strong>;
                })}
                {i < (msg.content || '').split('\n').length - 1 && '\n'}
              </span>
            );
        ...
        {isEditing && (
          <div style={{ marginTop: 6, display: 'flex', flexDirection: 'column', gap: 6 }}>
            <textarea
              ...
              style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: '1.5px solid #C6E404', background: 'var(--bg-page)', color: 'var(--text-primary)', fontSize: 14, resize: 'none', fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }}
              rows={2}
              autoFocus
            />
            <div style={{ display: 'flex', gap: 6, fontSize: 11 }}>
              <button onClick={onEditSave} style={{ padding: '4px 10px', borderRadius: 6, background: '#C6E404', color: '#1A1A1A', border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: 11 }}>Save</button>
            ...
            {Object.entries(reactions).map(([emoji, users]) => (
              <button key={emoji} onClick={() => onReact(msg.id, emoji)}
                style={{
                  ...
                  background: users.includes(myId) ? 'rgba(198,228,4,0.08)' : 'var(--bg-raised)',
...
export default function WorkspacePage() {
  ...
  // Load live operations activity feed
  const loadActivityFeed = useCallback(async () => {
    if (!account?.id) return;
    const tok = JSON.parse(localStorage.getItem('plex_auth_session')||'{}')?.access_token;
    try {
      const r = await fetch(`/api/workspace/activity-feed?account_id=${account.id}&limit=20`, {
  ...
  React.useEffect(() => { loadActivityFeed(); }, [loadActivityFeed]);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 8px' }}>
          ...
          {showNewChannel && (
            <div style={{ padding: '8px', marginBottom: 4 }}>
              <div style={{ display: 'flex', gap: 6 }}>
                ...
                <button onClick={createChannel} style={{ background: '#C6E404', border: 'none', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', color: '#1A1A1A' }}>
                  <Check size={14} />
                </button>
              </div>
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 10px' }}>
            <div style={{ position: 'relative' }}>
              <div style={{ width: 26, height: 26, borderRadius: 6, background: '#C6E404', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#1A1A1A', fontSize: 11, fontWeight: 800 }}>
                {myName.charAt(0).toUpperCase()}
              </div>
              <div style={{ position: 'absolute', bottom: -2, right: -2, width: 9, height: 9, borderRadius: '50%', background: '#C6E404', border: '2px solid #0F172A' }} title="Online" />
            </div>
          ...
          {activeMembersList.map(m => {
            ...
            const presenceColor = presenceStatus === 'online' ? '#C6E404' : presenceStatus === 'away' ? '#64748B' : '#6B7280';
            ...
            <>
              ...
              {pendingInvites.map(m => (
                 <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 10px', borderRadius: 8 }}>
                   <div style={{ width: 26, height: 26, borderRadius: 6, background: 'rgba(217,119,6,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748B', fontSize: 11, fontWeight: 800 }}>
        ...
        <div style={{ padding: '12px 12px calc(8px + env(safe-area-inset-bottom))', borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ width: 32, height: 32, borderRadius: 8, background: '#C6E404', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#1A1A1A', fontSize: 13, fontWeight: 800, flexShrink: 0 }}>
            {myName.charAt(0).toUpperCase()}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, background: 'var(--bg-page)', overflow: 'hidden' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 6, flexShrink: 0, alignItems: 'center' }}>
            {/* Live ops feed button */}
            <button onClick={() => setShowActivityFeed(p => !p)}
              title="Live Operations Feed"
              style={{ background: showActivityFeed ? 'rgba(255,255,255,0.15)' : 'none', border: 'none', cursor: 'pointer', padding: 6, borderRadius: 8, color: showActivityFeed ? '#fff' : 'rgba(255,255,255,0.6)', fontSize: 15 }}>
              ⚡
            </button>
            {/* Notification bell */}
            <div style={{ position: 'relative' }}>
            ...
            <button onClick={() => setShowInvite(true)}
              style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 8, border: 'none', background: 'var(--accent, #C6E404)', color: '#fff', cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', paddingTop: 8 }}>
          ...
          {activeChannel && groupedMessages.length === 0 && !loadingMsgs && (
            <div style={{ textAlign: 'center', padding: '48px clamp(12px,4vw,24px)', color: 'var(--text-muted)' }}>
          ...
          <div style={{ position:'absolute', bottom:'100%', left:16, right:16, background:'var(--bg-surface)', border:'1px solid var(--border)', borderRadius:12, overflow:'hidden', boxShadow:'0 8px 32px rgba(11,18,32,0.15)', zIndex:50 }}>
            {mentionSuggestions.map(p => (
              ...
              onMouseLeave={e => e.currentTarget.style.background='none'}>
                <div style={{ width:28, height:28, borderRadius:7, background:'#C6E404', display:'flex', alignItems:'center', justifyContent:'center', color:'#1A1A1A', fontSize:12, fontWeight:800, flexShrink:0 }}>
          ...
          <textarea
            ...
            <button
              ...
              style={{
                ...
                background: input.trim() && !sending && activeChannel ? '#C6E404' : 'var(--border)',
      ...
      {showInvite && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(11,18,32,0.6)', zIndex: 500, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', padding: 0 }}
          ...
          <div style={{
            ...
            {inviteStatus && (
              <div style={{ padding: '8px 12px', borderRadius: 8, background: inviteStatus.includes('✅') ? 'rgba(34,197,94,0.1)' : 'rgba(239,68,68,0.1)', color: inviteStatus.includes('✅') ? '#C6E404' : '#dc2626', fontSize: 13, marginBottom: 12 }}>
                {inviteStatus}
              </div>
            )}
            <button onClick={inviteMember} style={{ width: '100%', padding: '13px', background: '#C6E404', color: '#1A1A1A', border: 'none', borderRadius: 12, fontSize: 15, fontWeight: 800, cursor: 'pointer' }}>
              Send Invitation
            </button>
                ...
                {pendingInvites.map(m => (
                  <div key={m.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: '#64748B' }}>⏳ Pending · {m.role}</div>
                    </div>
                    <button onClick={() => removeMember(m.id, true)}
      ...
      {showActivityFeed && (
        <div style={{ position:'fixed', top:0, right:0, bottom:0, width:320, background:'var(--bg-surface)', borderLeft:'1px solid var(--border)', zIndex:300, display:'flex', flexDirection:'column', boxShadow:'-8px 0 32px rgba(0,0,0,0.18)' }}>
          <div style={{ padding:'14px 18px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between', background:'linear-gradient(135deg,#C6E40410,#C6E40408)' }}>
            <div>
            ...
            <button onClick={() => setShowActivityFeed(false)} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--text-muted)', fontSize:20, padding:4, lineHeight:1 }}>✕</button>
          </div>
          <div style={{ flex:1, overflowY:'auto' }}>
            {activityFeed.length === 0 ? (
              <div style={{ textAlign:'center', padding:48 }}>
                <div style={{ fontSize:32, marginBottom:10 }}>⚡</div>
                <p style={{ fontSize:13, fontWeight:600, color:'var(--text-primary)', margin:'0 0 4px' }}>No activity yet</p>
                <p style={{ fontSize:12, color:'var(--text-muted)', margin:0 }}>Business events will appear here as they happen</p>
              </div>
            ) : activityFeed.map((item, idx) => (
              <div key={idx} style={{ display:'flex', gap:10, padding:'10px 14px', borderBottom:'0.5px solid var(--border)' }}>
                <div style={{ width:30, height:30, borderRadius:9, background:String(item.color||'#6B7280')+'18', display:'flex', alignItems:'center', justifyContent:'center', fontSize:14, flexShrink:0 }}>
                  {String(item.icon||'•')}
                </div>
                <div style={{ flex:1, minWidth:0 }}>
                  <p style={{ margin:0, fontSize:12, fontWeight:700, color:'var(--text-primary)', lineHeight:1.4 }}>{String(item.title||'')}</p>
                  {item.desc ? <p style={{ margin:'1px 0 0', fontSize:11, color:'var(--text-muted)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{String(item.desc)}</p> : null}
                  {item.time ? <p style={{ margin:'2px 0 0', fontSize:10, color:'var(--text-muted)' }}>{new Date(item.time).toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit'})}</p> : null}
                </div>
              </div>
            ))}
          </div>
