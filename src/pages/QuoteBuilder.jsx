...
function PriceCell({ value, onChange, disabled }) {
  return (
    <div className="relative">
      <span className="absolute left-2 top-1/2 -translate-y-1/2 text-ink-subtle text-xs pointer-events-none select-none">$</span>
      <input
        type="number"
        value={value}
        min={0}
    ...
    <div className={`svc-row ${isSelected ? 'active' : ''}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-ink-muted mt-0.5 leading-relaxed">{svc.desc}</p>
          {isSelected && billingMode === 'annual' && monthlyPrice > 0 && !isIncluded && (
            <p className="text-xs font-semibold mt-1" style={{ color: XERO }}>
        ...
        {isSelected && (
          <div className="flex items-center gap-3 shrink-0 flex-wrap justify-end">
            ...
            <div className="flex flex-col items-end gap-1">
              ...
              {isIncluded
                ? <span className="text-xs font-semibold text-green-600 w-[86px] text-right">Included</span>
                ...
                : (
                  <div className="flex flex-col items-end">
                    ...
                    {billingMode === 'annual' && monthlyPrice > 0 && (
                      <span className="text-xs mt-0.5 font-semibold" style={{ color: XERO }}>
...
function Section({ section, services, selected, included, prices, billingMode, yearlyDiscount, onToggle, onPriceChange, onIncludeChange }) {
  ...
  return (
    <div className={`card overflow-hidden mb-3 ${isWeb ? 'border-xero-200' : ''}`}>
      <button onClick={() => setOpen(o => !o)} className="section-trigger">
        <div
          className="w-7 h-7 rounded flex items-center justify-center shrink-0"
          style={{ background: isWeb ? XERO : '#1a1a1a' }}
        >
          <Icon size={14} className="text-white" />
        </div>
        <span className="text-sm font-semibold text-ink flex-1">{section.label}</span>
        {count > 0 && (
          <span className="text-xs font-bold px-2 py-0.5 rounded text-white" style={{ background: XERO }}>
            {count}
          </span>
        )}
      ...
      {open && isWeb && (
        <div className="mx-5 mt-0 mb-2 px-3 py-2 rounded text-xs text-ink-muted flex items-start gap-2 border" style={{ background: '#f0fafd', borderColor: '#baeaf9' }}>
          <Info size={12} className="shrink-0 mt-0.5" style={{ color: XERO }} />
          <span>Select a <strong>website build</strong> (one-time) and optionally add a <strong>management plan</strong> (monthly retainer). Can be combined with any automation services.</span>
        </div>
      )}
...
function computeTotals({ selected, included, prices, billingMode, yearlyDiscount, discType, discValue, discSetup, discMonthly }) {
...
export default function QuoteBuilder() {
  ...
  const [notes, setNotes] = useState(
    'Pricing valid for 30 days. Monthly billing starts after setup is complete. Setup begins within 48 hours of signed agreement and initial deposit. No long-term contracts on monthly services — cancel anytime with 30 days notice.'
  );
  ...
  const { setupSub, mthSub, setupDiscAmt, mthDiscAmt, setupFinal, mthFinal, selectedIds } = computeTotals(fullState);
  const selectedCount = selectedIds.length;

  return (
    <div className="min-h-screen" style={{ background: '#F5F7F8' }}>

      {/* ── Header — white with Xero Blue accents ── */}
      <header className="bg-white border-b sticky top-0 z-40 no-print" style={{ borderColor: '#E5E8EB' }}>
        <div className="max-w-7xl mx-auto px-5 h-14 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            {/* Xero-style logo: blue rounded square */}
            <div
              className="w-8 h-8 rounded flex items-center justify-center font-bold text-white text-sm select-none"
              style={{ background: XERO }}
            >
              P
            </div>
            <div className="flex items-baseline gap-2">
              <span className="font-bold text-ink text-sm tracking-tight">{agency.agencyName}</span>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs text-ink-muted hover:text-ink px-3 py-1.5 rounded border transition-colors"
        ...
        {/* Xero-style blue underline bar */}
        <div className="h-0.5 w-full" style={{ background: XERO }} />
      </header>

      {/* ── Agency settings drawer ── */}
      {showSettings && (
        <div className="bg-white border-b no-print" style={{ borderColor: '#E5E8EB' }}>
          <div className="max-w-7xl mx-auto px-5 py-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-semibold text-ink flex items-center gap-2">
                <Settings size={14} style={{ color: XERO }} />
                Agency settings
              </span>
              <button onClick={() => setShowSettings(false)} className="text-ink-muted hover:text-ink">
                <X size={15} />
              </button>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                { key: 'agencyName', label: 'Agency name', ph: 'PLEX Automation' },
                { key: 'agencyEmail', label: 'Email', ph: 'hello@plex.io' },
                { key: 'agencyPhone', label: 'Phone', ph: '256-000-0000' },
                { key: 'agencyWebsite', label: 'Website', ph: 'plexautomation.io' },
              ].map(f => (
                <div key={f.key}>
                  <label className="text-xs text-ink-muted block mb-1 font-medium">{f.label}</label>
                  <input
                    type="text"
                    value={agency[f.key]}
                    placeholder={f.ph}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5">
          ...
          <div>
            <div className="mb-5">
              ...
              <p className="text-sm text-ink-muted mt-0.5">Select services, configure billing, adjust pricing — then export or send.</p>
            </div>

            {/* Client info */}
            <div className="card p-5 mb-4">
              <div className="flex items-center gap-2 mb-4 pb-3 border-b" style={{ borderColor: '#E5E8EB' }}>
                <Users size={14} style={{ color: XERO }} />
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  { v: clientBiz,  s: setClientBiz,  label: 'Business name', ph: 'McLain Team SIP' },
                  { v: clientEmail,s: setClientEmail, label: 'Email address',  ph: 'jane@business.com', type: 'email' },
                  { v: clientPhone,s: setClientPhone, label: 'Phone number',   ph: '(256) 000-0000' },
            ...
            <div className="card p-5 mb-4">
              <div className="flex items-center gap-2 mb-4 pb-3 border-b" style={{ borderColor: '#E5E8EB' }}>
                <Calendar size={14} style={{ color: XERO }} />
                ...
                {[
                  ...
                  return (
                    <button
                      ...
                      className="relative flex flex-col items-start p-4 rounded text-left transition-all border-2"
                      style={{
                        borderColor: active ? XERO : '#E5E8EB',
                        background: active ? '#f0fafd' : '#FFFFFF',
                        borderRadius: '8px',
                      }}
                    >
                      {opt.badge && (
                        <span className="absolute top-3 right-3 text-xs font-bold px-2 py-0.5 rounded text-white" style={{ background: XERO }}>
                          {opt.badge}
                        </span>
                      )}
                      <div className="flex items-center gap-2 mb-1.5">
                        <opt.Icon size={14} style={{ color: active ? XERO : '#7A7E85' }} />
                        <span className="text-sm font-semibold" style={{ color: active ? XERO : '#1a1a1a' }}>{opt.title}</span>
                        {active && <CheckCircle size={13} style={{ color: XERO }} />}
                      </div>
              ...
              {billingMode === 'annual' && (
                <div className="flex items-center gap-3 px-4 py-3 rounded text-sm border" style={{ background: '#f0fafd', borderColor: '#baeaf9', borderRadius: '6px' }}>
                  <Info size={13} style={{ color: XERO }} className="shrink-0" />
                  <span className="text-xs text-ink-muted flex-1">Discount applied to all monthly line items:</span>
                  <div className="flex items-center gap-2">
                    <input
                      type="number" min={1} max={50} value={yearlyDiscount}
                      ...
                      style={{ borderColor: XERO, color: XERO, borderRadius: '5px' }}
                    />
            ...
            <div className="card p-5">
              <div className="flex items-center gap-2 mb-3 pb-3 border-b" style={{ borderColor: '#E5E8EB' }}>
                <FileText size={14} style={{ color: XERO }} />
              ...
              <textarea
                ...
                placeholder="Payment terms, start dates, or any conditions..."
              />
            </div>
          </div>
          ...
          <div>
            <div className="sticky top-20 space-y-3">
              ...
              <div className="card overflow-hidden">
                {/* Xero-style blue header strip */}
                <div className="px-5 py-3 flex items-center justify-between" style={{ background: XERO }}>
                  <span className="text-sm font-bold text-white flex items-center gap-2">
                    <TrendingUp size={14} />
                    Quote summary
                  </span>
                  {selectedCount > 0 && (
                    <span className="text-xs font-bold bg-white px-2 py-0.5 rounded" style={{ color: XERO }}>
                ...
                <div className="p-5">
                  {selectedCount === 0 ? (
                    <div className="text-center py-6">
                      <div className="w-10 h-10 rounded mx-auto mb-3 flex items-center justify-center" style={{ background: '#e8f8fd' }}>
                        <Zap size={18} style={{ color: XERO }} />
                      </div>
                      <p className="text-sm font-medium text-ink">No services selected</p>
                      <p className="text-xs text-ink-muted mt-1">Check items from the list to build your quote</p>
                    </div>
                  ) : (
                    <>
                      {/* Billing mode pill */}
                      <div className="inline-flex items-center gap-1.5 text-xs font-semibold mb-4 px-2.5 py-1 rounded border"
                        style={{ background: '#f0fafd', borderColor: '#baeaf9', color: XERO }}>
                        <Calendar size={11} />
                        ...
                        <div className="space-y-1.5">
                          ...
                          {setupDiscAmt > 0 && (
                            <div className="flex justify-between text-sm">
                              ...
                              <span className="font-semibold tabular-nums" style={{ color: XERO }}>−{fmt(setupDiscAmt)}</span>
                            </div>
                          )}
                      ...
                      <div className="mb-4 pb-4 border-b" style={{ borderColor: '#E5E8EB' }}>
                        ...
                        <div className="space-y-1.5">
                          ...
                          {mthDiscAmt > 0 && (
                            <div className="flex justify-between text-sm">
                              <span className="text-ink-muted">Discount</span>
                              <span className="font-semibold tabular-nums" style={{ color: XERO }}>−{fmt(mthDiscAmt)}</span>
                            </div>
                          )}
                          <div className="flex justify-between items-baseline pt-1.5 border-t" style={{ borderColor: '#E5E8EB' }}>
                          ...
                          {billingMode === 'annual' && mthFinal > 0 && (
                            <div className="flex justify-between items-center mt-1 px-3 py-2 rounded text-xs font-semibold"
                              style={{ background: '#f0fafd', color: XERO, borderRadius: '6px' }}>
                          ...
                          {selectedIds.map(id => {
                            ...
                            return svc ? (
                              <li key={id} className="flex items-start gap-1.5 text-xs text-ink-muted">
                                <CheckCircle size={11} className="shrink-0 mt-0.5" style={{ color: XERO }} />
              ...
              <div className="card p-4">
                ...
                <div className="flex gap-2 mb-3">
                  {[{ k: 'pct', label: '% Percent' }, { k: 'flat', label: '$ Fixed' }].map(t => (
                    <button
                      ...
                      style={{
                        background: discType === t.k ? XERO : '#FFFFFF',
                        color: discType === t.k ? '#FFFFFF' : '#7A7E85',
                        borderColor: discType === t.k ? XERO : '#E5E8EB',
                        borderRadius: '5px',
                      }}
                    >
                      {t.label}
                ...
                <div className="flex items-center gap-2 mb-3">
                  <input
                    type="number" value={discValue} min={0}
                    onChange={e => setDiscValue(e.target.value)}
                    className="field text-right w-24 font-semibold tabular-nums"
                    placeholder="0"
                  />
                ...
                <div className="flex gap-4">
                  {[
                    { key: 'discSetup', label: 'Setup', val: discSetup, set: setDiscSetup },
                    { key: 'discMonthly', label: 'Monthly', val: discMonthly, set: setDiscMonthly },
                  ].map(opt => (
                    <label key={opt.key} className="flex items-center gap-1.5 text-xs text-ink-muted cursor-pointer">
                      <input type="checkbox" checked={opt.val} onChange={e => opt.set(e.target.checked)} style={{ accentColor: XERO }} />
                      {opt.label}
                    </label>
                  ))}
                </div>
              ...
              <div className="space-y-2">
                <button onClick={() => exportPDF(fullState)} disabled={selectedCount === 0} className="btn-primary w-full">
                  <Download size={15} />
                  Export PDF quote
                </button>
                <button onClick={() => openMailto(fullState)} disabled={selectedCount === 0} className="btn-secondary w-full">
                  <Mail size={15} />
      ...
      <footer className="border-t mt-8 py-4 no-print" style={{ borderColor: '#E5E8EB', background: '#FFFFFF' }}>
        <div className="max-w-7xl mx-auto px-5 flex items-center justify-between">
          <span className="text-xs text-ink-muted">{agency.agencyName} · {agency.agencyWebsite} · {agency.agencyPhone}</span>
          <span className="text-xs text-ink-muted">Quote Builder v2.1</span>
        </div>
      </footer>
    </div>
  );
