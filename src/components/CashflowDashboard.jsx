...
export default function CashflowDashboard({ accountId, accent = '#C6E404' }) {
  ...
  const summaryCards = [
    ...
    { label: 'Next 60 days',  value: summary.next_60   ?? 0, color: '#C6E404' },
    { label: 'Next 90 days',  value: summary.next_90   ?? 0, color: '#C6E404' },
  ];
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={weeks12} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <Bar dataKey="amount" radius={[4,4,0,0]}>
                {weeks12.map((w, i) => <Cell key={i} fill={i === 0 ? '#ef4444' : i <= 2 ? accent : '#C6E404'} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        )}
            ...
            <RePieChart>
              <Pie data={weeks12.filter(w => w.amount > 0)} dataKey="amount" nameKey="label" cx="50%" cy="50%" outerRadius={80} label={({name, value}) => `${name}: $${Math.round(value).toLocaleString()}`} labelLine={false}>
                {weeks12.filter(w => w.amount > 0).map((w, i) => (
                  <Cell key={i} fill={[accent, '#C6E404', '#C6E404', '#64748B', '#ef4444', '#C6E404', '#C6E404', '#C6E404', '#10b981', '#64748B', '#06b6d4', '#C6E404'][i % 12]} />
                ))}
              </Pie>
                ...
                <div className="space-y-1">
                  {client_profiles.map((p, i) => (
                    <div key={i} className="flex items-center gap-3 text-xs py-1">
                      ...
                      <span className="font-semibold" style={{ color: p.avg_dtp <= 7 ? '#C6E404' : p.avg_dtp <= 30 ? accent : '#ef4444' }}>
